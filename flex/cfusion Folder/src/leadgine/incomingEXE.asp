...
sub processData()
	...
	if myType = "table" then
		sqlLeads = "Select Referrals.ID,Referrals.Company,Referrals.myEvent,Referrals.Arrival, Referrals.Sent_Date,Referrals.TotalRooms,Referrals.RmRevenue + Referrals.RentalRevenue + Referrals.CateringRevenue as Revenue,Referrals.Bonus,Referrals.myStatus from Referrals_Hotels, Referrals where Referrals.ID = Referrals_Hotels.Referral_ID and Referrals_Hotels.Property_ID = " & hotel & " order by Sent_Date,Company,myEvent "
		...
		for x = 11 to 0 step -1
			...
			sqlLeads = "Select Count(Referrals.ID) as myCount from Referrals_Hotels, Referrals where Referrals.ID = Referrals_Hotels.Referral_ID and Referrals_Hotels.Property_ID = " & hotel & " and month(Referrals.sent_date)= " & myMonth & " and year(Referrals.sent_date)=" & myYear
			set rsLeads = myConnSQL.Execute(sqlLeads)
			call TrapIt()
