<cfcomponent output="false">
	...
	<cfscript>
		...
		variables.dao = CreateObject("component", "com.adobe.leadgine.cfc.ARTDAO");
	</cfscript>

	<cffunction name="fill" output="no" returntype="com.adobe.leadgine.cfc.ART[]" access="remote">
	...
	<cffunction name="get" output="no" returnType="com.adobe.leadgine.cfc.ART" access="remote">
		...
		<cftry>
			...
			<cfelseif ArrayLen(ret) GT 1>
				<cfset msg="get returned more than one record when ARTID had a value of #key#.">
				<cfthrow message="#msg#">
			<cfelse>
				<cfset msg="get did not return anything when ARTID had a value of #key#.">
				<cfthrow message="#msg#">
			</cfif>
	...
	<cffunction name="doUpdate" access="private" output="no">
		...
		<cftry>
			...
			<cfcatch type="conflict">
				<cfset co.conflict(variables.dao.read(id=new.getARTID()))>
			</cfcatch>
