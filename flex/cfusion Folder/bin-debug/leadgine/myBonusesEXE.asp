...
sub processData()
	...
	if myType = "table" then
		sqlLeads = "Select ID,Company,myEvent,Arrival, Sent_Date,TotalRooms,RmRevenue + RentalRevenue + CateringRevenue as Revenue,Bonus,myStatus from Referrals where RefUserID = " & usr & " and myStatus='DEF' and Checked_Out = 1 order by Sent_Date,Company,myEvent "
		...
		for x = 11 to 0 step -1
			...
			sqlLeads = "Select Sum(Bonus) as myCount from Referrals where RefUserID = " & usr & " and month(sent_date)=" & myMonth & " and year(sent_date)=" & myYear & " and myStatus='DEF' and Checked_Out = 1 "
			set rsLeads = myConnSQL.Execute(sqlLeads)
			call TrapIt()
