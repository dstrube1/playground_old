...
--  07/01/11 JRC  SIP-7721 Add Excluded Publishers filter to FiltertypeID=6 (Virtual Groups)
--	07/08/11 MRS  If Client is in EasternTimeZone don't use fn_ConvertTime 
...
JOIN CampaignSETitleIDAdIDMapping am  ON am.CampaignID = cac.CampaignID 
      ...
      AND am.SEID = rp.SEID
      AND am.AdID = rp.ADID
WHERE rp.ClientID = ' + CAST(@ClientID AS VARCHAR(10)) +'
