...
DECLARE @LatencySec INT,
	...
	WHERE OBJECT_NAME(i.object_id) = 'TrackedCache'
	AND ROWS > 0
	
	
	
SELECT @LatencySec = DATEDIFF(S,MAX(CreatedDate),GETUTCDATE())
FROM TrackedCache WITH (NOLOCK)
WHERE $Partition.PF_SI_TrackedHistory(EasternCreateDate) = @PNUM
