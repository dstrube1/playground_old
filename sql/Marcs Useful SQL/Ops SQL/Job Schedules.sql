...
-- Set variables
--***************************************************************************************
set @StartDT = CAST(getdate() - 1 AS DATE) 
...
SELECT  @cols = REPLACE(  -- Replace the space in the XML path by a comma
        ...
        ( 
            ...
            SELECT      quotename(col1) AS [data()] 
            FROM  (SELECT DISTINCT col1 FROM #Pivot ) s
            ORDER BY col1 
            FOR XML PATH ('') 
        ), '] [', '],[') 
...
SET @sql = N'     
                        SELECT col2 as SP,' + @cols + N' 
                        FROM #Pivot
                        PIVOT(MAX(col3) FOR col1 IN(' + @cols + N')) AS P
                        order by 1'
