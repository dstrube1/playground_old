...
--------------------------------------------------------------
  ...
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO Backup Delete'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
...
--------------------------------------------------------------
  ...
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO DB Backup'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--------------------------------------------------------------
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Integrity Checks')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO Integrity Checks'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--------------------------------------------------------------
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Optimizations')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO Optimizations'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--this part takes care of the old way of doing thnigs
  -- Delete the job with the same name (if it exists)
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Optimizations Pre')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error deleting job ''EPRO Optimizations Pre'' since there is a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Optimizations Post')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error deleting job ''EPRO Optimizations Post'' since there is a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--------------------------------------------------------------
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Rebuild Constraints')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO Rebuild Constraints'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--------------------------------------------------------------
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Transaction Log Backup')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error creating job ''EPRO Transaction Log Backup'' since there is already a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
...
--this part takes care of the old way of doing thnigs
  ...
  SELECT @DelJobID = job_id     
  FROM   msdb.dbo.sysjobs    
  WHERE (name = N'EPRO Transaction Log Backup PRE/POST')       
  IF (@DelJobID IS NOT NULL)    
  BEGIN  
  -- Check if the job is a multi-server job  
  IF (EXISTS (SELECT  * 
              ...
              WHERE   (job_id = @DelJobID) AND (server_id <> 0))) 
  BEGIN 
    RAISERROR (N'Error deleting job ''EPRO Transaction Log Backup PRE/POST'' since there is a multi-server job with this name.', 16, 1) 
  END 
  ELSE 
    -- Delete the [local] job 
    EXECUTE msdb.dbo.sp_delete_job @job_id = @DelJobID 
