...
def validation(validate,input,validation_type,player)
  ...
  until validate
    ...
    when 'game_move'
      ...
      input = gets.chomp
      validate = is_valid_move?(input.to_i)
    end
    validation_end_msg(validation_type,input,player)
  end
end

def validation_end(validation_type,input,player)
